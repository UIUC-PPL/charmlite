...
#define MAX_ITER 100
#define MAX_CHARE 10000

class cmk_tuple : std::tuple<int, int>
{
    operator+=(cmk_tuple t)
    {
...
//namespace cmk {
    template <>
    struct cmk::index_view<cmk_tuple>
    {
        static cmk_tuple decode(chare_index_t& idx)
        {
            return std::make_tuple(idx % MAX_CHARE, idx / MAX_CHARE);
        }

        static const std::tuple<int, int> decode(const chare_index_t& idx)
        {
            return std::make_tuple(idx % MAX_CHARE, idx / MAX_CHARE);
        }

        static chare_index_t encode(const std::tuple<int, int>& idx)
        {
            return std::get<1>(idx) * MAX_CHARE + std::get<0>(idx);
        }
    };
//}
...
struct ghost_message : public cmk::message
{
    ...
    ghost_message(int direction_, int size_)
        : cmk::message(cmk::message_helper_<ghost_message>::kind_,
                sizeof(message) + 2 * sizeof(int) + size_ * sizeof(double))
          , direction(direction_)
          , size(size_)
          , data((char*) this + 2 * sizeof(int) + sizeof(message))
    {
    }
...
public:
    ...
    void begin_iteration() 
    {
        ...
        double* ghost_data;
        int msg_size_x = sizeof(cmk::message) + 2 * sizeof(int) + block_dim_x * sizeof(double);
        int msg_size_y = sizeof(cmk::message) + 2 * sizeof(int) + block_dim_y * sizeof(double);
        ...
        if(!left_bound)
        {
            ...
            this_proxy[std::make_tuple(std::get<0>(this_index) - 1, std::get<1>(this_index))].send<
        ...
        if(!top_bound)
        {
            ...
            this_proxy[std::make_tuple(std::get<0>(this_index), std::get<1>(this_index) - 1)].send<
                ghost_message, &jacobi::receive_ghosts>(std::move(msg));
        }
        if(!bottom_bound)
        {
    ...
    void compute() 
    {
        ...
        this_proxy[this_index].contribute<completion_message, cmk::nop>(
                std::move(msg), cb);
    }
    ...
    {
        CmiPrintf("\n\n[%d,%d]\n",std::get<0>(this_index), std::get<1>(this_index));
        for(int i = 0; i < block_dim_x + 2; ++i)
        {
            for(int j = 0; j < block_dim_y + 2; ++j)
...
int main(int argc, char** argv)
{
    ...
    if (CmiMyNode() == 0)
    {
        ...
        auto msg = cmk::make_message<setup_message>(array_dim_x, array_dim_y,
                block_dim_x, block_dim_y, num_chare_x, num_chare_y, maxiterations);
        auto array = cmk::collection_proxy<jacobi>::construct();

        for(auto i = 0; i < num_chare_x; i++)
            for(auto j = 0; j < num_chare_y; j++)
                array[std::make_tuple(i, j)].insert(msg);
